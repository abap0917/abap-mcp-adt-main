...
exports.handleCustomizingApply = handleCustomizingApply;
exports.handleCustomizingCreate = handleCustomizingCreate;
exports.handleCustomizingStatus = handleCustomizingStatus;
...
const applyTool = {
    ...
    description: '[customizing] Copy (or delete) customizing rows from sourceKey to targetKey through the SM30 maintenance-view runtime (foreign-key checks, change documents, governed transport recording — exactly like a manual SPRO change). DRY RUN by default; commit: true applies.',
    inputSchema: {
        type: 'object',
        properties: {
            object_name: {
            ...
            target_key: {
                type: 'string',
                description: 'Target org-unit value (for action delete: the key to delete).',
            },
            action: {
                type: 'string',
                enum: ['copy', 'delete'],
            ...
            only_missing: {
                type: 'boolean',
                description: 'copy: skip keys already present in target (default true).',
                default: true,
            },
            values: {
                type: 'array',
                description: 'Optional [{field, value}] overrides applied to every copied row.',
                items: {
                    type: 'object',
            ...
            transport: {
                type: 'string',
                description: 'Customizing transport request or task (W function). If omitted and create_transport is false, an interactive prompt is returned instead of writing.',
            },
            create_transport: {
                type: 'boolean',
            ...
            no_transport: {
                type: 'boolean',
                description: 'Write WITHOUT a transport request (skips transport governance; the engine routes by client capability — on a non-recording client like this one, changes go through the SM30 view runtime without transport recording). Use only for dev/test or when SCC4 does not record customizing changes.',
                default: false,
            },
            auto_deploy: {
                type: 'boolean',
                description: 'Redeploy the engine if missing or stale before writing (default true).',
                default: true,
            },
            package_name: {
                type: 'string',
...
const createTool = {
    ...
    available_in: ['onprem', 'legacy'],
    description: '[customizing] Write explicit customizing rows through the engine (SM30 view runtime). DRY RUN by default; commit: true applies. Rows are [{field, value}, …] per row.',
    inputSchema: {
        type: 'object',
        properties: {
            object_name: {
            ...
            rows: {
                type: 'array',
                description: 'Explicit rows; each row is an array of {field, value} (full key + data).',
                items: {
                    type: 'array',
                    items: {
                        type: 'object',
...
const statusTool = {
    ...
    available_in: ['onprem', 'legacy'],
    description: '[customizing] Poll the result of an async customizing commit/delete by its run_id (returned by CustomizingApply / CustomizingCreate / OrgCopy when a commit is still pending).',
    inputSchema: {
        type: 'object',
        properties: {
            run_id: {
...
function formatEngineResult(res, verb) {
    const lines = [
...
async function handleCustomizingApply(context, args) {
    ...
    try {
        ...
        const maint = await (0, resolveMaint_1.resolveMaint)(connection, logger, args.object_name);
        ...
        if (args.no_transport) {
            ...
            const d = await (0, transportGovernance_1.resolveTransport)(connection, logger, {
        ...
        const valuesJson = args.values && args.values.length
            ? JSON.stringify(args.values.map((v) => ({
        ...
        if (valuesJson)
            req.values_json = valuesJson;
        const op = action === 'delete' ? 'delete' : 'write';
        const res = await (0, engineClient_1.callEngine)(connection, logger, op, req);
        const text = formatEngineResult(res, action === 'delete' ? 'Customizing delete' : 'Customizing copy');
        return {
...
async function handleCustomizingCreate(context, args) {
    const { connection, logger } = context;
    try {
        if (!args?.object_name || !Array.isArray(args.rows)) {
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, 'object_name and rows are required');
        }
        const commit = args.commit === true;
        ...
        const decision = await (0, transportGovernance_1.resolveTransport)(connection, logger, { transport: args.transport, createTransport: args.create_transport }, `MCP cust ${maint.rootTable} create`);
        ...
        const rowsJson = JSON.stringify(args.rows.map((row) => row.map((c) => ({ FIELD: c.field.toUpperCase(), VALUE: c.value }))));
        const req = {
            table: maint.rootTable,
            view_name: maint.view ?? '',
            transport_object: maint.recordObject,
        ...
        if (decision.kind === 'create')
            req.create_transport = 'X';
        const res = await (0, engineClient_1.callEngine)(connection, logger, 'create', req);
...
async function handleCustomizingStatus(context, args) {
    const { connection, logger } = context;
    try {
        if (!args?.run_id)
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, 'run_id is required');
        const res = await (0, engineClient_1.callEngine)(connection, logger, 'status', { run_id: args.run_id }, 30_000);
...
exports.TOOL_DEFINITIONS = [applyTool, createTool, statusTool];
//# sourceMappingURL=handleCustomizingWrite.js.map