...
exports.describeAdtError = describeAdtError;
exports.deployClass = deployClass;
exports.deployReport = deployReport;
...
function describeAdtError(err) {
    const base = err?.message ?? String(err);
    const data = err?.response?.data;
    if (!data)
        return base;
    const s = typeof data === 'string' ? data : JSON.stringify(data);
    const msg = s.match(/<(?:[a-zA-Z0-9_]+:)?message[^>]*>([^<]+)</) ||
...
const LOCK_ACCEPT = 'application/vnd.sap.as+xml;charset=UTF-8;dataname=com.sap.adt.lock.result;q=0.8, ' +
 ...
 */
async function updateInPlace(context, kind, name, source, transport) {
    const { connection, logger } = context;
    const lower = name.toLowerCase();
    const base = kind === 'class'
    ...
    try {
        ...
        const xml = typeof lockResp.data === 'string' ? lockResp.data : String(lockResp.data);
        lockHandle = xml.match(/<LOCK_HANDLE>([^<]+)<\/LOCK_HANDLE>/)?.[1];
        if (!lockHandle) {
            throw new Error(`Lock failed for ${name} (no LOCK_HANDLE): ${xml.slice(0, 300)}`);
        }
        const corrNr = xml.match(/<CORRNR>([^<]*)<\/CORRNR>/)?.[1] ?? transport ?? '';
        logger?.info(`updateInPlace: ${name} locked (corrNr=${corrNr || '(none)'})`);
        ...
        if (put.status >= 400) {
            const body = typeof put.data === 'string'
        ...
        if (d.includes('lock') || d.includes('锁定')) {
            throw new Error(`Cannot update ${name}: the object is already locked (open SE24/SE80 editor or a lock entry in a transport request). ` +
        ...
        try {
            ...
            logger?.warn(`⚠️  FAILED to unlock ${name} (lock may be left behind — release in SE03): ${describeAdtError(unlockErr)}`);
        }
        connection.setSessionType('stateless');
    }
}
async function deployClassObject(context, name, source, pkg, transport) {
    const { connection, logger } = context;
    ...
    try {
        await adtClass.create({
            ...
            description: `MCP in-system customizing engine (v${engineMeta_js_1.ENGINE_VERSION})`,
            sourceCode: source,
        ...
        const exists = msg.includes('already exist') ||
...
async function deployReportObject(context, name, source, pkg, transport) {
    const { connection, logger } = context;
    const client = (0, clients_js_1.createAdtClient)(connection, logger);
    ...
    try {
        await program.create({
            ...
            packageName: pkg,
            transportRequest: transport,
            description: `MCP customizing engine background writer (v${engineMeta_js_1.ENGINE_VERSION})`,
            sourceCode: source,
        }, { activateOnCreate: false });
...
async function deployClass(context, name, source, opts = {}) {
    return deployClassObject(context, name, source, (opts.package_name ?? '$TMP').toUpperCase(), opts.transport_request?.trim().toUpperCase());
}
async function deployReport(context, name, source, opts = {}) {
    return deployReportObject(context, name, source, (opts.package_name ?? '$TMP').toUpperCase(), opts.transport_request?.trim().toUpperCase());
}
