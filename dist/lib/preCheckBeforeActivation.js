 ...
 */
function compilerQuirkHint(realErrors) {
    const joined = realErrors.map((e) => e.text).join(' ');
    const quoted = `['"\u201c\u201d][^'"\u201c\u201d]*['"\u201c\u201d]`;
    const matches = new RegExp(`(?:statement|语句)\\s+${quoted}\\s+(?:is invalid|无效)`, 'i').test(joined) ||
        /(?:unable to interpret|无法解释)/i.test(joined) ||
        /(?:fully typed|完全类型化)/i.test(joined) ||
        /(?:statement is not accessible|语句不可访问|statement is not defined|语句未定义)/i.test(joined) ||
        new RegExp(`(?:statement|语句)\\s+${quoted}\\s+(?:is not allowed|不允许)`, 'i').test(joined);
    if (!matches)
        return '';
    return (" — This SAP system's compiler rejects some standard constructs: decimal literals like `0.6` " +
...
function assertNoCheckErrors(result, kind, name) {
    ...
    const full = realErrors
        ...
        .join(' | ');
    const message = `${kind} ${name} preCheck syntax check failed (${realErrors.length} error${realErrors.length === 1 ? '' : 's'}): ${full}${compilerQuirkHint(realErrors)}`;
