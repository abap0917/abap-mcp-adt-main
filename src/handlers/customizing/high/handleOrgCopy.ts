...
export const TOOL_DEFINITION = {
  ...
  description:
    '[customizing] Headless copy of an organizational unit (company code, plant, sales org, …) and its dependent customizing — the "org units in the dark" copier (EC01-style). Records on a Customizing transport the governed way. DRY RUN by default; commit: true applies.',
  inputSchema: {
    type: 'object',
    properties: {
      org_unit: {
        ...
        description:
          'Org-unit type/domain to copy (BUKRS = company code, WERKS = plant, …).',
      },
      ...
      no_transport: {
        type: 'boolean',
        description:
          'Copy WITHOUT a transport request (skips transport governance; engine routes by client capability — non-recording client → no transport recording). Dev/test only.',
        default: false,
      },
      auto_deploy: {
        type: 'boolean',
        description:
          'Redeploy the engine if missing or stale before copying (default true).',
        default: true,
      },
      package_name: {
        type: 'string',
...
export async function handleOrgCopy(
  context: HandlerContext,
  args: OrgCopyArgs,
) {
  const { connection, logger } = context;
  try {
    for (const k of ['org_unit', 'source_key', 'target_key'] as const) {
      if (!args?.[k])
    ...
    if (args.no_transport) {
      ...
      const d = await resolveTransport(
        ...
        `MCP org copy ${args.org_unit} ${args.source_key}->${args.target_key}`,
      );
      if (d.kind === 'prompt') {
        return {
          isError: false,
          content: [{ type: 'text' as const, text: d.prompt }],
        };
      }
    ...
    const res = await callEngine(connection, logger, 'org_copy', req);
    const lines: string[] = [
    ...
    if (res.RUN_ID) {
      ...
      lines.push(
        '  Commit still running — poll with CustomizingStatus (run_id)',
      );
    }
    ...
    if (res.STATUS === 'error') {
      lines.push(
        '',
        'Note: org copy requires the ECOP entity copier (S/4HANA) — check CustomizingEnginePing for HAS_ORG_COPY.',
      );
    }
