...
import {
  loadRowsFor,
  resolveBaseTable,
} from '../../../lib/customizing/resolveMaint';
import type { SqlRow } from '../../../lib/customizing/runSql';
...
const diffTool = {
  ...
  description:
    '[customizing] Compare customizing rows between two org-unit key values of the same config object (e.g. company code 1000 vs 2000).',
  inputSchema: {
    type: 'object',
    properties: {
      object_name: {
...
const planTool = {
  ...
  available_in: ['onprem', 'legacy'] as const,
  description:
    '[customizing] Dry-run plan of a config copy: the exact rows that would be written when copying sourceKey → targetKey (with optional field overrides). Nothing is written.',
  inputSchema: {
    type: 'object',
    properties: {
      object_name: {
      ...
      values: {
        type: 'array',
        description:
          'Optional [{field, value}] overrides applied to every planned row.',
        items: {
          type: 'object',
...
function rowKey(row: SqlRow, nonOrgFields: string[]): string {
...
function formatRows(rows: SqlRow[]): string {
  ...
  for (const r of rows) {
    lines.push(
      cols.map((c, i) => (r[c] ?? '').slice(0, 40).padEnd(widths[i])).join(' '),
    );
  }
  return lines.join('\n');
}

async function loadSides(context: HandlerContext, args: CompareArgs) {
  ...
  const [srcRows, tgtRows] = await Promise.all([
    loadRowsFor(connection, logger, baseTable, kf, src),
    loadRowsFor(connection, logger, baseTable, kf, tgt),
  ]);
  return { baseTable, kf, src, tgt, srcRows, tgtRows };
}

export async function handleCustomizingDiff(
  ...
  try {
    if (
      ...
      throw new McpError(
        ...
        'object_name, key_field, source_key, target_key are required',
      );
    }
    const { baseTable, kf, src, tgt, srcRows, tgtRows } = await loadSides(
      context,
      args,
    );
    ...
    const onlyInSrc = srcRows.filter(
      (r) => !tgtIndex.has(rowKey(r, nonOrgFields)),
    );
    const onlyInTgt = tgtRows.filter(
      (r) => !srcIndex.has(rowKey(r, nonOrgFields)),
    );
...
export async function handleCustomizingPlanChange(
  context: HandlerContext,
  args: CompareArgs,
) {
  const { logger } = context;
  try {
    ...
    const planned = srcRows.map((r) => {
      const row: SqlRow = { ...r, [kf]: tgt };
      for (const [f, v] of overrides) row[f] = v;
      return row;
    });
...
export const TOOL_DEFINITIONS = [diffTool, planTool];
