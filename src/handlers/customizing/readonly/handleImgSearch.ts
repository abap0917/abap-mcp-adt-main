...
import { callEngine } from '../../../lib/customizing/engineClient';
import {
  col,
  runSql,
  type SqlRow,
  sql1,
} from '../../../lib/customizing/runSql';
...
export const TOOL_DEFINITION = {
  ...
  description:
    '[customizing] Search SAP IMG/SPRO activities by keyword (activity title). Prefers the STREE search-text index when the customizing engine is deployed; falls back to the raw CUS_IMGACT tables. Pass a namespace (e.g. "/POSDW/") to scope the raw search — free-text SQL is blocked on this system.',
  inputSchema: {
    type: 'object',
    properties: {
      keyword: {
      ...
      namespace: {
        type: 'string',
        description:
          'Activity-ID prefix to scope the raw search by, e.g. "/POSDW/" (omit for best-effort full scan).',
      },
      in_scope_only: {
        type: 'boolean',
        description:
          "Keep only activities in the client's activated scope (CUS_IMGACH_SCOPE).",
        default: false,
      },
      language: {
        type: 'string',
...
async function imgSearchRaw(
  ...
  let scanned: SqlRow[] = [];
  try {
    scanned = await runSql(connection, logger, sql, max);
  } catch {
    // CUS_IMGACT/CUS_ACTOBJ not query-accessible
  }

  let rows = kw
    ? scanned.filter((r) => col(r, 'TEXT').toUpperCase().includes(kw))
    : scanned;

  let scopeNote = '';
  if (args.in_scope_only && ns) {
    const scopeRows = await sql1(
      connection,
      logger,
  ...
  for (const r of rows) {
    ...
    lines.push(`${act} ${title} ${type} ${obj} ${col(r, 'TCODE')}`);
  }
  lines.push(
    '',
    'Use CustomizingDescribe <object> to see the table set, key fields, and transport object.',
    'source: CUS_IMGACT raw IMG tables (no STREE search index)',
  );
...
async function imgSearchViaIndex(
  ...
  const genRows = await runSql(
    connection,
    logger,
    `SELECT TREE_ID, SPRAS, GEN_DATE FROM TTREESRCH ORDER BY GEN_DATE DESCENDING`,
    100,
  );
  if (genRows.length === 0) return null;

  const genByTree = new Map<string, string>();
  ...
  const imgRows = await sql1(
    connection,
    logger,
    `SELECT ID FROM TTREE WHERE TYPE = 'IMG' AND ID IN (${inList})`,
  );
  const imgIds = new Set(imgRows.map((r) => col(r, 'ID')));
  const trees = ids.filter((id) => imgIds.has(id));
  if (trees.length === 0) return null;

  const hits: Array<{ TEXT: string; PATH?: string; EXTENSION?: string }> = [];
  const sources: string[] = [];
  for (const t of trees.slice(0, 3)) {
    let res:
      | import('../../../lib/customizing/engineClient').EngineResponse
      | undefined;
    try {
      res = await callEngine(connection, logger, 'img_index_read', {
        structure_id: t,
        language: lang,
        keyword: kw,
        max_rows: max,
    ...
    for (const h of parsed) hits.push(h);
  ...
  if (hits.length === 0) {
    return (
      `No IMG nodes matching "${args.keyword}".\n` +
  ...
  lines.push(
    '',
    `source: ${sources.join('; ')}  (index used — vs. raw IMG tables)`,
    'Use CustomizingDescribe <object> for the table set + transport object.',
  );
  return lines.join('\n');
}

export async function handleImgSearch(
    ...
    if (args.keyword.trim()) {
      try {
        ...
        if (viaIndex) {
          return {
            isError: false,
            content: [{ type: 'text' as const, text: viaIndex }],
          };
        }
      } catch (err) {
        logger?.debug(
          'ImgSearch: index path failed, falling back to raw IMG tables',
          err as any,
        );
      }
    }
