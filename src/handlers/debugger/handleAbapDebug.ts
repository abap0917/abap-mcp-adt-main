...
// ---------------------------------------------------------------------------
// Session state (in-memory, per connection)
// ---------------------------------------------------------------------------
...
function sessionKey(connection: IAbapConnection): string {
...
function getOrCreateSession(
  ...
  user: string,
): { key: string; session: DebugSession } {
...
async function getConnectionUser(connection: IAbapConnection): Promise<string> {
  ...
  try {
    ...
    if (ctx.responsible) return ctx.responsible;
  } catch {
    /* ignore */
  }
  return 'USER';
...
export const TOOL_DEFINITIONS = [
  {
    ...
    description:
      '[debug] Manage an ABAP debugging session: listen (register a debugger listener and wait for a breakpoint to be hit), status, detach.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
          ...
          description:
            'listen: wait for breakpoint hit; status: show current state; detach: end session.',
        },
        username: {
          type: 'string',
  ...
  } as const,
  {
    ...
    available_in: ['onprem', 'legacy'] as const,
    description:
      '[debug] Set a line breakpoint in an ABAP program. Returns the breakpoint id(s), usable with AbapDebugDeleteBreakpoint.',
    inputSchema: {
      type: 'object',
      properties: {
        source_url: {
          type: 'string',
          description:
            'ADT source URL of the object, e.g. /sap/bc/adt/programs/programs/Z_MY_PROG/source/main.',
        },
        line: {
          type: 'number',
  ...
  } as const,
  {
    ...
    available_in: ['onprem', 'legacy'] as const,
    description:
      '[debug] Delete a breakpoint by its id (from AbapDebugSetBreakpoint).',
    inputSchema: {
      type: 'object',
      properties: {
        breakpoint_id: {
    ...
    inputSchema: {
      ...
      properties: {
        step_type: {
          ...
          description:
            'stepInto (F5), stepOver (F6), stepReturn (F7), stepContinue (F8), terminateDebuggee.',
        },
      },
      required: ['step_type'],
    },
  } as const,
  {
    ...
    available_in: ['onprem', 'legacy'] as const,
    description:
      '[debug] Inspect variable values in the active ABAP debug session.',
    inputSchema: {
      type: 'object',
      properties: {
        variables: {
          ...
          items: { type: 'string' },
          description:
            'Variable names, e.g. ["LV_COUNT", "WA_MARA", "IT_TABLE"].',
        },
      },
      required: ['variables'],
    },
  } as const,
  {
    ...
    available_in: ['onprem', 'legacy'] as const,
    description:
      '[debug] Get the current call stack in the active ABAP debug session.',
  ...
  } as const,
  {
    ...
    available_in: ['onprem', 'legacy'] as const,
    description:
      '[debug] Change the value of a variable in the active ABAP debug session.',
    inputSchema: {
      type: 'object',
      properties: {
        variable_name: {
...
function respText(resp: any): string {
  const d = resp?.data;
...
function fail(err: any, logger?: ILogger, label = 'debug tool') {
...
export async function handleAbapDebugSession(
  context: HandlerContext,
  args: { action: 'listen' | 'status' | 'detach'; username?: string },
) {
  ...
  try {
    ...
    if (args.action === 'status') {
      ...
      try {
        ...
        return ok(
          `Active debug session: ${session ? `terminal=${session.terminalId} ide=${session.ideId} user=${session.user}` : 'none registered in this server'}\n` +
            `\nSAP listener status:\n${respText(resp)}`,
        );
      } catch (e: any) {
        return ok(
          `Session state: ${session ? 'registered' : 'none'}\nSAP listener query failed: ${e.message}`,
        );
      }
    }
    ...
    if (
      ...
      return ok(
        `Listener registered (terminal ${session.terminalId}). No breakpoint hit within the wait window yet.\n` +
...
export async function handleAbapDebugSetBreakpoint(
  context: HandlerContext,
  args: { source_url: string; line: number; username?: string },
) {
  const { connection, logger } = context;
  try {
    if (!args?.source_url || !args.line) {
      throw new McpError(
        ...
        'source_url and line are required',
      );
    }
    const user = args.username ?? (await getConnectionUser(connection));
    const { session } = getOrCreateSession(connection, user);
    ...
    return ok(
      `🔴 Breakpoint set at ${args.source_url} line ${args.line}.\n${idLine}\n${bodyText}`,
    );
  } catch (error: any) {
...
export async function handleAbapDebugDeleteBreakpoint(
  context: HandlerContext,
  args: { breakpoint_id: string; username?: string },
) {
  const { connection, logger } = context;
  try {
    if (!args?.breakpoint_id)
      throw new McpError(ErrorCode.InvalidParams, 'breakpoint_id is required');
    const user = args.username ?? (await getConnectionUser(connection));
    const { session } = getOrCreateSession(connection, user);
    await req(
      connection,
      logger,
      `/sap/bc/adt/debugger/breakpoints/${encodeURIComponent(args.breakpoint_id)}`,
      'DELETE',
      {
        params: {
          scope: 'external',
...
export async function handleAbapDebugStep(
  context: HandlerContext,
  ...
  try {
    if (!args?.step_type)
      throw new McpError(ErrorCode.InvalidParams, 'step_type is required');
...
export async function handleAbapDebugVariable(
  context: HandlerContext,
  ...
  try {
    if (!Array.isArray(args?.variables) || args.variables.length === 0) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'variables (non-empty array) is required',
      );
    }
...
export async function handleAbapDebugStack(context: HandlerContext) {
  const { connection, logger } = context;
  try {
    const resp = await req(
      connection,
...
export async function handleAbapDebugSetVariable(
  context: HandlerContext,
  args: { variable_name: string; value: string },
) {
  const { connection, logger } = context;
  try {
    if (!args?.variable_name || args.value === undefined) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'variable_name and value are required',
      );
    }
    const resp = await req(connection, logger, '/sap/bc/adt/debugger', 'POST', {
