...
export const TOOL_DEFINITION = {
  ...
  description:
    '[quality] ATC (ABAP Test Cockpit) findings. Provide object_url to run a new check, run_id to fetch an existing run worklist by its worklist ID, or display_id to fetch a saved ATC result by display ID. Returns priority, check, message, location.',
  inputSchema: {
    type: 'object',
    properties: {
      object_url: {
      ...
      run_id: {
        type: 'string',
        description:
          'Existing ATC run worklist ID to fetch findings for (skip run creation).',
      },
      display_id: {
        type: 'string',
        description:
          'Saved ATC result display ID to fetch findings for (from the ATC results list, e.g. "59D32AF578641FE1A5EE3789C160133D").',
      },
      variant: {
        type: 'string',
 ...
 */
function parseWorklistId(wlXml: string): string | null {
  const asAttr = xmlAttr(wlXml, 'worklist', 'id');
  const asElement = wlXml.match(
    /<[a-zA-Z0-9_:]*:id[^>]*>([^<]+)<\/[a-zA-Z0-9_:]*:id>/,
  );
  const id = asAttr ?? asElement?.[1];
  if (!id || id === EMPTY_WORKLIST_ID) return null;
  return id;
}
 ...
 */
function parseRunId(runXml: string): string | null {
  const asElement = runXml.match(
    /<[a-zA-Z0-9_:]*worklistId[^>]*>([^<]+)<\/[a-zA-Z0-9_:]*worklistId>/,
  );
 ...
 */
function formatWorklist(wlXml: string, maxResults: number): ParsedResult {
  const objectBlocks = [
    ...wlXml.matchAll(
  ...
  for (const [, , openAttrs, inner] of objectBlocks) {
    const objName =
      xmlAttr(openAttrs, undefined, 'name') ??
    ...
    for (const [, , attrs] of findings) {
      findingCount++;
      if (findingCount > maxResults) continue;
      ...
      const line =
        attrs.match(/\bline="(\d+)"/)?.[1] ?? location.split('line=')[1] ?? '';
...
function formatResultHeader(xml: string): string {
  const get = (tag: string) => {
    const m = xml.match(
      new RegExp(`<atcresult:${tag}>([^<]*)</atcresult:${tag}>`),
    );
    return m ? m[1] : '';
  };
  const parts = [
...
export async function handleRunAtcAnalysis(
  ...
  try {
    ...
    const modeCount = [args?.object_url, args?.run_id, args?.display_id].filter(
      Boolean,
    ).length;
    if (modeCount === 0) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Provide one of object_url, run_id or display_id.',
      );
    }
    if (modeCount > 1) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Provide only one of object_url, run_id or display_id.',
      );
    }
    ...
    if (args?.display_id) {
      ...
      const { text, findingCount, objectCount } = formatWorklist(
        wlXml,
        maxResults,
      );
      if (findingCount === 0) {
