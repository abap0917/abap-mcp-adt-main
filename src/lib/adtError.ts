...
export function parseAdtException(xml: string): AdtException | null {
  if (!/<[a-zA-Z0-9_]*:?exception[^>]*>/i.test(xml)) return null;
  const type =
    xml.match(/<[a-zA-Z0-9_]*:?type\b[^>]*id="([^"]+)"/)?.[1] ??
    xml.match(/<[a-zA-Z0-9_]*:?type[^>]*>([^<]+)</)?.[1];
  const namespace = xml.match(
    /<[a-zA-Z0-9_]*:?namespace\b[^>]*id="([^"]+)"/,
  )?.[1];
  const message =
    xml.match(/<[a-zA-Z0-9_]*:?message\b[^>]*>([^<]+)</)?.[1] ?? '';
  const localizedMessage =
    xml.match(/<[a-zA-Z0-9_]*:?localizedMessage\b[^>]*>([^<]+)</)?.[1] ?? '';
...
function responseBody(err: any): string | undefined {
...
export function isNotFoundError(err: any): boolean {
  ...
  const body = responseBody(err);
  if (!body) return status === 422;
  const exc = parseAdtException(body);
  if (!exc) return status === 422;
