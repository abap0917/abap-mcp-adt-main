...
/**
 * Customizing/debug tools that mutate configuration, deploy ABAP or drive the
 * debugger — blocked on QA/PRD like the Create/Update/Delete prefixes (their
 * names don't carry those prefixes, so they are listed explicitly).
 */
const TIER_GUARDED_TOOLS: ReadonlySet<string> = new Set([
  // customizing writes / engine lifecycle (deploy = repository write)
  'CustomizingApply',
  'CustomizingCreate',
  'OrgCopy',
  'CustomizingEngineBootstrap',
  'CustomizingEngineCleanup',
  'CustomizingEngineSelftest',
  // debugger (executes/attaches to ABAP runtime)
  'AbapDebugSession',
  'AbapDebugSetBreakpoint',
...
export function checkToolAllowed(toolName: string, tier: Tier): string | null {
  ...
  if (TIER_GUARDED_TOOLS.has(toolName)) {
    return `${toolName} mutates SAP objects / executes on the server; only DEV profiles may run it.`;
  }
