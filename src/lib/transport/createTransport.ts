...
import { col, runSql } from '../customizing/runSql';
...
export function sanitizeTarget(t: string): string {
 ...
 */
export async function discoverTransportTargets(
  connection: IAbapConnection,
  logger: ILogger | undefined,
): Promise<TransportTargets> {
  ...
  try {
    const rows = await runSql(
      connection,
      logger,
      `SELECT TARSYSTEM, COUNT(*) AS CNT FROM E070 WHERE TARSYSTEM <> '' GROUP BY TARSYSTEM`,
      500,
    );
    for (const r of rows) {
      const t = sanitizeTarget(col(r, 'TARSYSTEM'));
      if (!t) continue;
      if (!targets.includes(t)) targets.push(t);
      freq[t] = (freq[t] ?? 0) + (parseInt(col(r, 'CNT'), 10) || 1);
    }
  } catch {
    // GROUP BY unsupported → fall back to DISTINCT (may be capped by row limit).
    try {
      const rows2 = await runSql(
        connection,
        logger,
        `SELECT DISTINCT TARSYSTEM FROM E070`,
        500,
      );
...
function parseTransportResponse(xml: string): Partial<CreatedTransport> {
