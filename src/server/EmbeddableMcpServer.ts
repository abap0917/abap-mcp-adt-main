...
import { CustomizingHandlersGroup } from '../lib/handlers/groups/CustomizingHandlersGroup.js';
import { DebugHandlersGroup } from '../lib/handlers/groups/DebugHandlersGroup.js';
...
export class EmbeddableMcpServer extends BaseMcpServer {
  ...
  private createDefaultRegistry(
    ...
    if (exposition.includes('customizing')) {
      groups.push(new CustomizingHandlersGroup(dummyContext));
    }
    if (exposition.includes('debug')) {
      groups.push(new DebugHandlersGroup(dummyContext));
    }
